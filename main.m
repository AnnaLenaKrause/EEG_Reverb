...
savedir = 'D:\EEG_Reverb\StimuliNew_RWTH_symm'; % save directory
nrand = 3;
randN = {'A','B','C'};
fs = 48000;
rms = .08;
...
for r = 1:nrand
    ...
    for block = 1 : length(seq)
        ...
        if max(abs(stim))>0.95
            sprintf(['pay attention: clipping in file B%i_S&i_' RANDN{r}],block,0)
        end
        ...
        for stix = 1 :  size(seq{block},2)
            ...
            % warning for clipping
            if max(abs(stim))>0.95
                sprintf(['pay attention: clipping in file B%i_S&i_' RANDN{r}],block,stix)
            end
